...
import { Menu, Moon, Sun, Search, Plus, Heart, ChevronDown, Check, LayoutGrid, BookOpen, Book, Film, Tv, Gamepad2, Loader2, ArrowUpDown, Clapperboard, X, Edit2, Trash2 } from 'lucide-react';
    ...
    <div className="flex min-h-screen w-full overflow-hidden bg-earth-50 dark:bg-[#191919] transition-colors duration-300 font-sans">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="flex justify-between items-center px-6 py-4 z-20">
            <button onClick={() => setSidebarOpen(true)} className="p-3 bg-white dark:bg-stone-800 shadow-sm border border-stone-200 dark:border-stone-700 rounded-xl text-earth-600 dark:text-earth-300 hover:scale-105 transition-all">
              <Menu size={20} />
            </button>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-12 pb-12 custom-scrollbar">
          <div className="max-w-6xl mx-auto w-full">
              ...
              <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
                <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto">
                  <div className="flex items-center gap-1 overflow-x-auto pb-2 xl:pb-0 hide-scrollbar max-w-full">
                    {categoriesList.map(cat => {
                      ...
                      return (
                        <button 
                          key={cat.id} onClick={() => setSelectedCategory(cat.id as any)}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-sm whitespace-nowrap transition-all border ${isSelected ? 'bg-white dark:bg-stone-800 border-stone-300 dark:border-stone-600 text-earth-800 dark:text-stone-100 shadow-sm font-medium' : 'border-transparent text-stone-500 hover:bg-stone-100 dark:hover:bg-stone-800'}`}
                        >
                  ...
                  <div className="relative" ref={ratingDropdownRef}>
                    ...
                    {isRatingDropdownOpen && (
                      <div className="absolute top-full left-0 mt-2 w-48 bg-white dark:bg-stone-800 rounded-md shadow-xl border border-stone-200 dark:border-stone-700 py-1 z-50 overflow-hidden">
                         {ratingOptions.map((option) => (
                           <button key={option.id} onClick={() => { setSelectedRating(option.id as any); setIsRatingDropdownOpen(false); }}
                             className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 ${selectedRating === option.id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'text-stone-700 dark:text-stone-300 hover:bg-stone-50'}`}
                           >
                ...
                <div className="flex items-center gap-2 w-full xl:w-auto mt-2 xl:mt-0 flex-nowrap">
                  <div className="flex-1 xl:w-48 relative group min-w-[120px]">
                    ...
                    <input type="text" placeholder="搜尋..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-8 pr-3 py-1.5 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-sm outline-none placeholder:text-stone-300" />
                  </div>
                  
                  <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-1.5 px-4 py-1.5 bg-[#5e5045] dark:bg-stone-700 text-white rounded text-sm hover:bg-[#4a403a] transition-colors whitespace-nowrap shadow-sm shrink-0">
                    <Plus size={16} /> <span>新增</span>
                  </button>

                  <div className="relative shrink-0" ref={sortDropdownRef}>
                    <button onClick={() => setIsSortDropdownOpen(!isSortDropdownOpen)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-sm whitespace-nowrap transition-all border ${isSortDropdownOpen ? 'bg-white dark:bg-stone-800 border-stone-300 dark:border-stone-600 text-earth-800 shadow-sm' : 'border-transparent text-stone-500 hover:bg-stone-100'}`}>
                      <ArrowUpDown size={14} />
                      <span className="hidden sm:inline">{sortOptions.find(o => o.id === sortBy)?.label || '排序'}</span>
                      <ChevronDown size={14} className={`${isSortDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {isSortDropdownOpen && (
                      <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-stone-800 rounded-md shadow-xl border border-stone-200 dark:border-stone-700 py-1 z-50 overflow-hidden">
                         {sortOptions.map((option) => (
                           <button key={option.id} onClick={() => { setSortBy(option.id as any); setIsSortDropdownOpen(false); }}
                             className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 ${sortBy === option.id ? 'bg-stone-100 dark:bg-stone-700 text-stone-900 dark:text-white font-medium' : 'text-stone-700 dark:text-stone-300 hover:bg-stone-50'}`}
                           >
            ...
            {/* --- 更換後的：小卡片樣式 (Grid Content) --- */}
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-32 text-stone-400"><Loader2 size={32} className="animate-spin mb-4" /><p>正在同步...</p></div>
            ) : processedEntries.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 text-stone-400"><Search size={32} className="mb-4 opacity-50" /><p>查無相符作品...</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {processedEntries.map((entry) => (
                  ...
                  >
                    ...
                    {entry.plurkUrl && (
                      <a href={entry.plurkUrl} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="absolute bottom-2 right-2 text-earth-300 dark:text-stone-500 hover:text-earth-500 p-1"><PlurkPIcon size={16} /></a>
                    )}
                  </div>
                ))}
              </div>
            ...
            {/* --- 更換後的：大卡片樣式 (Expanded View Modal) --- */}
            {expandedEntry && (
                    ...
                    <div className="flex flex-col gap-8">
                      <div className="flex flex-col sm:flex-row gap-4">
                        {expandedEntry.plurkUrl && (
                          <a href={expandedEntry.plurkUrl} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-3 py-5 bg-stone-800 dark:bg-stone-700 hover:bg-stone-900 text-white rounded-2xl font-bold transition-all shadow-xl text-lg"><PlurkPIcon size={20} /><span>前往噗文</span></a>
                        )}
                        <button onClick={() => setExpandedEntry(null)} className="px-10 py-5 border-2 border-stone-200 dark:border-stone-800 rounded-2xl hover:bg-stone-50 text-stone-600 font-bold text-lg">返回列表</button>
                      </div>
                      <div className="flex flex-wrap items-center justify-between gap-4 pt-8 border-t border-stone-200 dark:border-stone-800">
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => handleEdit(expandedEntry)} className="flex items-center gap-2 px-5 py-2.5 bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 rounded-xl text-sm font-bold"><Edit2 size={16} /> 編輯</button>
                          <button onClick={() => handleDelete(expandedEntry.id)} className="flex items-center gap-2 px-5 py-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-xl text-sm font-bold"><Trash2 size={16} /> 刪除</button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <footer className="mt-20 pb-12">
              <div className="bg-[#8b5e3c] dark:bg-stone-800 rounded-xl p-8 md:p-12 text-center text-[#fbf7f3] dark:text-stone-300 relative overflow-hidden shadow-lg">
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-3xl">
                     {[{ v: stats.total, l: '總收藏' }, { v: stats.manga, l: '漫畫' }, { v: stats.novel, l: '小說' }, { v: stats.movie, l: '電影' }].map(s => (
                       <div key={s.l} className="bg-white/10 rounded-lg p-4 backdrop-blur-sm flex flex-col items-center justify-center h-24 border border-white/10 hover:bg-white/20 transition-colors">
                          <span className="text-3xl font-bold text-white mb-1">{s.v}</span>
                          <span className="text-xs text-stone-100 tracking-wider font-medium opacity-80">{s.l}</span>
                       </div>
                     ))}
                  </div>
                </div>
